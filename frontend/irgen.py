...
class IRGen(ASTTransformer):
    ...
    def visitWhile(self, node):
        ...
        # for statements in the 'if' body are inserted before the 'else' body
        prefix = self.builder.block.name
        whileCond = self.add_block(prefix + '.whileCond')
    ...
    def visitArrayDef(self, node): #!todo does float array needs it own rules for init?
    ...
    def visitUnaryOp(self, node):
        # logical operators don't exist in LLVMs.stdout)

        if node.op == '!':
            if str(node.value.ty) != 'bool':
                print("weird type unary !.", file=sys.stdout)
                quit()
            eq = ast.Operator.get('==')
        ...
        if node.op == '-':
            if str(node.value.ty) == 'float':
                print("float negate", file=sys.stdout)

                return self.builder.fsub(self.getfloat(0), self.visit(node.value))
            else:
